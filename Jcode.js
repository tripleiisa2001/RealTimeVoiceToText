...
const languageSelect = document.getElementById("language");
let finalTranscript = "";
let isListening = false; // counter for continuous listening
...
// Typing effect in one line + auto-scroll
function typeAppend(el, text, speed = 10) {
  ...
  function typing() {
    if (i < text.length) {
      ...
      cursor.insertAdjacentText("beforebegin", " "); // space between words
      el.scrollTop = el.scrollHeight;
    }
  }
  typing();
...
recognition.onend = () => {
  if (isListening) {
    setTimeout(() => recognition.start(), 300); // short delay for mobile stability
  }
};
